...
abb_data_provision(filename_excel_ist);
...
events_all_double = cellfun(@str2double, matches);
% Erstes Ereignis extrahieren
first_event = events_all_double(1);
...
data_ist = [data_ist events_ist_double];
...
if pfp
    ...
    events_ist_double(events_ist_double == pfp_row) = 0;
    events_all_double(events_all_double == pfp_row) = []; 
end

% Finden der Zellen die Ereignisse beinhalten
index_segment = find(events_ist_double ~= 0);
...
index_trajectory = find(events_ist_double == first_event);
...
index_first_elements = find(events_all_double == first_event);
